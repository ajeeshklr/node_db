...
helpers.getFileContent = (dir, fileName) => fs.readFileSync(path.join(dir, fileName)).toString("base64");
...
helpers.getTemplate = function (templateName, data, callback) {
    ...
    if (templateName) {
        ...
        cacheControl.fetchTextFile(templatePath, 'utf8', function (err, str) {
...
helpers.interpolate = function (str, data) {
    ...
    for (var keyName in config.appConfig.templateGlobals) {
        if (config.appConfig.templateGlobals.hasOwnProperty(keyName)) {
            data['global.' + keyName] = config.appConfig.templateGlobals[keyName]
        }
    }
...
helpers.getStaticAsset = function (fileName, callback) {
    ...
    if (fileName) {
        ...
        cacheControl.fetchRawFile(publicDir + fileName, function (err, data) {
            if (!err && data) {
                callback(false, data);
            } else {
...
helpers.fetchGlobalHeaderInformation = function (templateData) {
    if (null == templateData) {
        templateData = {};
    }

    templateData['head.home'] = locale.strings.home_page;
    templateData['head.products'] = locale.strings.product_page;
    templateData['head.services'] = locale.strings.services_page;
    templateData['head.about_us'] = locale.strings.about_us;
    templateData['head.contact_us'] = locale.strings.contact_us;
...
helpers.printMemoryUsage = function () {
    //   NodeJS.global.gc();
